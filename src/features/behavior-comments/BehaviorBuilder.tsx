...
export function BehaviorBuilder({ classMode = false, profile, toast }: Props) {
  ...
  if (classMode && resultsOpen) return (
    <div className="mx-auto max-w-[1120px]">
      ...
      <div className="grid gap-3">{work.students.map((student) => <Card key={student.studentNumber} className="p-4 sm:p-6"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><b>{student.studentNumber}번 학생</b><StatusBadge status={student.status} /></div><p className="mt-2 whitespace-pre-wrap text-sm leading-relaxed text-ink">{student.finalText || "아직 생성된 행발 내용이 없습니다."}</p></div><Button size="sm" onClick={() => { selectStudent(student.studentNumber); setResultsOpen(false); }}>수정하기</Button></div></Card>)}</div>
  ...
  return (
    <div className="mx-auto max-w-[1120px]">
      ...
      {classMode && (
        <div className="mb-5 grid gap-3">
          ...
          <Card className="overflow-hidden p-0">
            <div className="grid items-center gap-4 p-4 sm:p-6 lg:grid-cols-[1fr_auto_1fr]">
            ...
            <div className="flex min-h-14 items-center justify-center gap-2 border-t border-line bg-primary-soft px-4 py-3 text-center">
      ...
      <section className="grid items-start gap-4 lg:grid-cols-[.82fr_1.18fr]"><KeywordPicker selected={active.selectedKeywords} onToggle={(keyword) => updateActive({ selectedKeywords: active.selectedKeywords.includes(keyword) ? active.selectedKeywords.filter((item) => item !== keyword) : [...active.selectedKeywords, keyword] })} /><div className="flex flex-col gap-4"><Card><h2 className="text-lg font-bold">관찰 수준 및 기록</h2><p className="mt-1 text-xs text-muted">키워드별 관찰 수준을 선택하고, 교사가 직접 관찰한 내용을 기록하세요.</p>{active.selectedKeywords.length ? <div className="mt-4 grid gap-2">{active.selectedKeywords.map((keyword) => <label key={keyword} className="grid grid-cols-[1fr_minmax(140px,.7fr)] items-center gap-3 border-b border-line pb-2"><b className="text-xs">{keyword}</b><select value={active.keywordLevels[keyword] ?? "잘함"} onChange={(event) => updateActive({ keywordLevels: { ...active.keywordLevels, [keyword]: event.target.value as BehaviorStudentWork["keywordLevels"][string] } })}>{behaviorDegree.map((degree) => <option key={degree}>{degree}</option>)}</select></label>)}</div> : <div className="grid min-h-[80px] place-items-center text-xs text-muted">왼쪽에서 키워드를 선택하세요.</div>}<label className="mt-4 block text-xs font-bold">교사가 직접 관찰한 내용<textarea className="mt-2 min-h-[125px] w-full font-normal" placeholder="예: 모둠 활동에서 친구의 의견을 끝까지 듣고 역할을 나누어 과제를 완성함." value={active.observationMemo} onChange={(event) => updateActive({ observationMemo: event.target.value })} /></label>{anonymized.redactions.length > 0 && <div className="mt-3 flex items-start gap-3 rounded-xl bg-success/10 p-3 text-success"><ShieldCheck size={17} className="shrink-0" /><div><b>기기 내 개인정보 {anonymized.redactions.length}건 가림</b><p className="mt-1 text-xs">{anonymized.text}</p></div></div>}<div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><label className="flex flex-col gap-2"><span className="text-xs font-bold">문체</span><select value={active.style} onChange={(event) => updateActive({ style: event.target.value })}>{styleOptions.map((option) => <option key={option}>{option}</option>)}</select></label><Button variant="primary" size="lg" disabled={!active.selectedKeywords.length && !active.observationMemo.trim()} onClick={() => void generate()}><Sparkles size={18} /> 행발 문장 생성</Button></div></Card>
      {active.snippets.length > 0 && <Card><div className="flex justify-between gap-3"><div><h2 className="text-lg font-bold">행발 초안</h2><p className="mt-1 text-xs text-muted">입력한 사실 밖의 내용이 없는지 확인하세요.</p></div><span className="h-fit rounded-full bg-warning/15 px-2 py-1 text-xs font-bold text-warning">교사 검토 필요</span></div><div className="my-4 grid gap-2">{active.snippets.map((snippet, index) => <div key={index} className="grid grid-cols-[28px_minmax(0,1fr)_44px] items-start gap-2"><span className="grid size-7 place-items-center rounded-lg bg-primary-soft text-xs font-bold text-primary-dark">{index + 1}</span><textarea className="min-h-[74px]" value={snippet} onChange={(event) => { const snippets = [...active.snippets]; snippets[index] = event.target.value; updateActive({ snippets, finalText: snippets.join(" ") }); }} /><IconButton aria-label={`${index + 1}번 문장 다시 생성`} title="다른 문장으로 다시 생성" onClick={() => regenerateSnippet(index)}><RefreshCw size={16} /></IconButton></div>)}</div><label className="block text-xs font-bold">완성 문단<textarea className="mt-2 min-h-[150px] w-full font-normal leading-relaxed" value={active.finalText} onChange={(event) => updateActive({ finalText: event.target.value })} /></label><div className="mt-2 flex flex-wrap items-center justify-between gap-2 text-xs text-muted"><span>{active.finalText.length}자 · {utf8Bytes(active.finalText)}바이트</span><span className="text-success">● 개인정보 가림 확인</span></div><div className="mt-4 flex flex-wrap justify-end gap-2"><Button onClick={() => updateActive({ finalText: active.snippets.join(" ") })}><RefreshCw size={16} /> 다시 합치기</Button><Button variant="primary" onClick={async () => { await navigator.clipboard.writeText(active.finalText); toast("행발 문단을 복사했습니다."); }}><Clipboard size={16} /> 복사하기</Button></div></Card>}</div></section>
      {pendingCount !== null && <Modal labelledBy="behavior-count-title"><Card className="w-full max-w-md"><h2 id="behavior-count-title" className="text-lg font-bold">인원수를 변경할까요?</h2><p className="mt-2 text-sm text-muted">인원수를 줄이면 선택 범위를 벗어난 학생의 작업 내용이 삭제됩니다. 계속할까요?</p><div className="mt-5 flex justify-end gap-2"><Button onClick={() => setPendingCount(null)}>취소</Button><Button variant="danger" onClick={confirmCountChange}>인원수 변경</Button></div></Card></Modal>}
    </div>
  );
}

function StatusBadge({ status }: { status: BehaviorStudentWork["status"] }) { const label = status === "completed" ? "작성 완료" : status === "draft" ? "작성 중" : "미작성"; const color = status === "completed" ? "bg-success/15 text-success" : status === "draft" ? "bg-warning/15 text-warning" : "border border-line bg-subtle text-muted"; return <span className={`inline-flex items-center gap-1 rounded-full px-2 py-1 text-xs font-bold ${color}`}><span className="size-1.5 rounded-full bg-current" />{label}</span>; }
function DownloadWarning({ total, missing, onCancel, onConfirm }: { total: number; missing: number; onCancel: () => void; onConfirm: () => void }) { return <Modal labelledBy="download-warning-title"><Card className="w-full max-w-md"><h2 id="download-warning-title" className="text-lg font-bold">미작성 학생이 있습니다</h2><p className="mt-2 text-sm text-muted">전체 {total}명 중 {missing}명의 행발이 아직 작성되지 않았습니다. 빈칸을 포함하여 내려받을까요?</p><div className="mt-5 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><Button onClick={onCancel}>계속 작성하기</Button><Button variant="primary" onClick={onConfirm}><Download size={17} /> 빈칸 포함 내려받기</Button></div></Card></Modal>; }
